...
namespace ppp
{
    ...
    {
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::label(const char* l)
        {
            _config.label = l;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::description(const char* l)
        {
            _config.label_description = l;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::counter(const ecs_counter_t* c)
        {
            _config.counter = c;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::time_index(int t)
        {
            _config.time_index = t;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::y_range(float min, float max)
        {
            _config.y_min = min;
            _config.y_max = max;
            _config.auto_fit = false;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::scale(float s, const char* u)
        {
            _config.scale = s;
            _config.unit = u;
            return *this;
        ...
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::height(float h)
        {
            _config.height = h;
            return *this;
        }
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::ticks(const std::vector<double>& t, const std::vector<std::string>& labels)
        {
            _config.ticks = t;
        ...
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::auto_fit(float margin)
        {
        ...
        //-------------------------------------------------------------------------
        counter_plot_builder& counter_plot_builder::shaded(float alpha)
        {
        ...
        //-------------------------------------------------------------------------
        counter_plot_config counter_plot_builder::build()
        {
            return _config;
        }
        ...
        void draw_counter_card(const counter_plot_config& config)
        {
            ...
            if (config.auto_fit && config.counter)
            {
                ...
                {
                    float v = static_cast<float>(config.counter->value[i]) * config.scale;
            ...
            if (ImPlot::BeginPlot("##counter_plot", ImVec2(-1, config.height), ImPlotFlags_NoLegend))
            {
                ...
                std::vector<float> scaled_values(ECS_STAT_WINDOW);
                for (int i = 0; i < ECS_STAT_WINDOW; ++i)
                {
                    scaled_values[i] = static_cast<float>(config.counter->value[i]) * config.scale;
                }

                if (config.show_shaded)
                {
                    ImPlot::PushStyleVar(ImPlotStyleVar_FillAlpha, config.shaded_alpha);
                    ImPlot::PlotShaded(config.label, scaled_values.data(), ECS_STAT_WINDOW);
                    ImPlot::PopStyleVar();
                }
                ImPlot::PlotLine(config.label, scaled_values.data(), ECS_STAT_WINDOW);

                ImPlot::EndPlot();
            }

            float val = static_cast<float>(config.counter->value[config.time_index]) * config.scale;
            float rate = static_cast<float>(config.counter->rate.avg[config.time_index]) * config.scale;

            ImGui::Text("Value: %.2f%s | Rate: %.2f%s", val, config.unit, rate, config.unit);
