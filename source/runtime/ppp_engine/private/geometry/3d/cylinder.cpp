...
namespace ppp
{
    ...
    {
        //-------------------------------------------------------------------------
        static void make_faces(geometry* self, bool top, bool bottom, s32 detail)
        {

        }
        //-------------------------------------------------------------------------
        static void make_vertices(geometry* self, s32 detail)
        {
            // input
            constexpr f32 _bottom_radius = 1.0f;
            constexpr f32 _top_radius = 1.0f;
            ...
            constexpr bool _bottom_cap = true;
            constexpr bool _top_cap = true;
            ...
            const bool bottom_cap = _bottom_cap;
            const bool top_cap = _top_cap || top_radius != 0;
        ...
        //-------------------------------------------------------------------------
        static void make_uvs(geometry* self, bool top, bool bottom, s32 detail)
        {
        ...
        geometry* make_cylinder(bool smooth_normals, bool top, bool bottom, s32 detail)
        {
            ...
            {
                auto create_geom_fn = [top, bottom, detail](geometry* self)
                {
                    //make_faces(self, top, bottom, detail);

                    make_vertices(self, detail);
                    //make_uvs(self, top, bottom, detail);
                    //make_normals(self);
                };
