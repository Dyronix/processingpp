...
namespace ppp
{
    namespace render
    {
        using unlit_composite_pass = graphics_unique_ptr<composite_pass<unlit_pass>>;
        using unlit_wireframe_composite_pass = graphics_unique_ptr<composite_pass<unlit_wireframe_pass>>;
        using forward_shading_composite_pass = graphics_unique_ptr<composite_pass<forward_shading_pass>>;
        using shadow_composite_pass = graphics_unique_ptr<composite_pass<shadow_pass>>;
        using predepth_composite_pass = graphics_unique_ptr<composite_pass<predepth_pass>>;

        //-------------------------------------------------------------------------
        template<typename T, typename TTag>
        static graphics_unique_ptr<composite_pass<T>> create_composite_pass(string::string_id pass_name, tag<TTag> shader_tag, string::string_id framebuffer_tag, s32 framebuffer_flags)
        {
            auto composite = memory::make_unique<composite_pass<T>, memory::persistent_graphics_tagged_allocator<composite_pass<T>>>(pass_name);

            composite->add_child(memory::make_unique<T,memory::persistent_graphics_tagged_allocator<T>>(decltype(shader_tag)::batched(), framebuffer_tag, framebuffer_flags, T::draw_mode::BATCHED));
            composite->add_child(memory::make_unique<T,memory::persistent_graphics_tagged_allocator<T>>(decltype(shader_tag)::instanced(), framebuffer_tag, framebuffer_flags, T::draw_mode::INSTANCED));
