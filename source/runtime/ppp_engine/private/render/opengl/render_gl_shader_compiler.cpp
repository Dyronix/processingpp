...
namespace ppp
{
    namespace render
    {
        namespace shaders
        {
            //-------------------------------------------------------------------------
            // shader compilation
            static bool compile_shader(GLuint* shader, GLenum type, const GLchar* source)
            {
                GLint status;
            ...
            u32 create_shader_program(std::string_view vs_source, std::string_view fs_source)
            {
                ...
                GLboolean res = compile_shader(&vert_shader, GL_VERTEX_SHADER, vs_source.data());
                if (!res)
                {
                ...
                res = compile_shader(&frag_shader, GL_FRAGMENT_SHADER, fs_source.data());
                if (!res)
                {
            ...
            u32 create_shader_program(std::string_view vs_source, std::string_view fs_source, std::string_view geometry_source)
            {
                ...
                res = compile_shader(&geom_shader, GL_GEOMETRY_SHADER, geometry_source.data());
                if (!res)
                {
