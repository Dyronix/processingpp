    ...
    namespace render
    {
        ...
        private:
            ...
            void copy_instance_data(s32 material_id, const glm::vec4& color, const glm::mat4& world)
            {
                ...
                scratch_vector<u8> instance_data(m_instance_buffer.element_size_in_bytes());
        ...
        instance_drawing_data::instance_drawing_data(const attribute_layout* layouts, u64 layout_count, const attribute_layout* instance_layouts, u64 instance_layout_count)
            : m_pimpl(memory::make_unique<impl, memory::persistent_global_tagged_allocator<impl>>(layouts, layout_count, instance_layouts, instance_layout_count))
        {
            assert(layouts != nullptr);
            assert(layout_count > 0);
