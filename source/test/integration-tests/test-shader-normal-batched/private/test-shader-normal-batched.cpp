    ...
    void end_draw()
    {
        if (environment::frame_count() == 5)
        {
            if (_generate_new_data)
            {
                ...
                image::save_pixels("local:/test-shader-normal-batched.png", _window_width, _window_height);
            }

            if (!_no_testing)
            {
                auto test_frame = image::load("local:/test-shader-normal-batched.png");
                ...
                {
                    environment::print("[TEST FAILED][SBNORMAL] image buffers are not identical!");
                }
                else
                {
                    environment::print("[TEST SUCCESS][SBNORMAL] image buffers are identical.");
                }
            }
