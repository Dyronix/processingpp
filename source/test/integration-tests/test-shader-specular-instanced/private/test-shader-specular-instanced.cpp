    ...
    void end_draw()
    {
        if (environment::frame_count() == 5)
        {
            if (_generate_new_data)
            {
                ...
                image::save_pixels("local:/test-shader-specular-instanced.png", _window_width, _window_height);
            }

            if (!_no_testing)
            {
                auto test_frame = image::load("local:/test-shader-specular-instanced.png");
