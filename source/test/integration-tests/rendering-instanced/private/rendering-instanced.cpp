...
namespace ppp
{
    ...
    void end_draw()
    {
        if (environment::frame_count() == 5)
        {
            ...
            image::save_pixels("local:/test-rendering-instanced.png", _window_width, _window_height);
#endif

#if PPP_CHECK_TEST_FRAME
            auto test_frame = image::load("local:/test-rendering-instanced.png");
            ...
            {
                environment::print("[TEST FAILED][RINST] image buffers are not identical!");
            }
            else
            {
                environment::print("[TEST SUCCESS][RINST] image buffers are identical.");
            }
#endif
