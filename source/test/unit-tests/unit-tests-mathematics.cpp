...
TEST_CASE("math::fract", "[math]") {
    using ppp::math::fract;
    // For a positive number, fract(x) should equal (x - floor(x)).
    double val = 3.14;
    double expected = val - std::floor(val);
    REQUIRE(fract(val) == Catch::Approx(expected));
    // For an integer, the fractional part should be zero.
    REQUIRE(fract(5.0) == 0.0);
}
